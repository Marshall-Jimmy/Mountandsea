...
│   └── full-data.json       # 完整版数据（全部山经+海经+大荒经，384实体）
│
├── tools/                   # 自动化校验工具
│   ├── kg_validator.py       # L1 数据格式校验器
│   ├── consistency_checker.py # L2 设定一致性检查器
│   ├── balance_simulator.py # L3 数值平衡仿真器
...
- [x] 知识图谱数据补全（全部山经+海经+大荒经，384实体，L1校验全通过）
- [x] 自动化校验工具开发（L1格式/L2一致性/L3数值仿真/L4风格，4个Python脚本）
- [x] CI/CD流程集成（GitHub Actions，推送/PR自动校验）
- [x] 数值平衡仿真器（科技树/材料经济/战力曲线/食物链四维仿真）
- [ ] 文言文风格检查器增强（接入大模型）
